...
export default {
  ...
  methods: {
    // 获取订单支付信息
    async getPayInfo() {
      let result = await this.$API.payOrderInfo(this.orderId);
      // 判断是否成功
      if (result.code == 200) {
    ...
    async open() {
      ...
      // 将二维码展示
      this.$alert(`<img src=${qrcode} />`, "请微信支付", {
        ...
        beforeClose: (type, instance, done) => {
          ...
          if (type == "confirm") {
            /* 
              其实这里应该比对code是否是200，如果是200才能执行下一步，但是为了方便下一步就跳过
            */
            // 关闭定时器
            clearInterval(this.timer);
            this.timer = null;
