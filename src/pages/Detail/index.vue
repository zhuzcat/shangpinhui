  ...
  methods: {
    changeActive(spuSaleAttrValue, List) {
      ...
      this.skuInfo.skuSaleAttrValueList.forEach((item) => {
        if (item.saleAttrId == spuSaleAttrValue.baseSaleAttrId) {
          item.saleAttrValueName = spuSaleAttrValue.saleAttrValueName;
        }
      });
    },
    valueChange() {
    ...
    // 添加到购物车
    async addToCart() {
      let params = { skuId: this.$route.params.skuid, skuNum: this.skuNum };
      // 这里的dispatch就是在执行addOrUpdataCart函数，而在vuex中action的函数为async返回值是一个promise实例，由此可以判断加入购物车是否成功
      // 然后利用try catch来捕获promise实例的错误，如果没有错误，就进行下一步的操作
      try {
        await this.$store.dispatch("addOrUpdataCart", params);
        // 将需要的对象信息存储在会话存储中
